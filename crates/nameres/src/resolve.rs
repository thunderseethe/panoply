  ...
  E: DiagnosticSink<NameResolutionError>,
{
    ...
    F: FnMut(&mut Self, &C) -> Option<D>,
  {
    Some(match row {
      ...
      cst::Row::Mixed {
        ...
        let variables = self.resolve_separated(variables, |me, var| me.resolve_type_symbol(*var));
        nst::Row::Mixed {
          concrete: concrete?,
          vbar: *vbar,
          variables: variables?,
  ...
  /// Resolves a type.
  pub fn resolve_type(&mut self, type_: Idx<cst::Type<Ident>>) -> Option<Idx<nst::Type<TyVarId>>> {
  ...
  pub fn resolve_term(&mut self, term: Idx<cst::Term>) -> Option<Idx<nst::Term>> {
    let term = match self.view(term) {
      ...
      cst::Term::ProductRow(pr) => {
        nst::Term::ProductRow(self.resolve_product_row(pr, |me, target| me.resolve_term(*target))?)
      }
      cst::Term::SumRow(sr) => {
  ...
  pub fn resolve_term_item(&mut self, id: TermName, term: &cst::TermDefn) -> Option<nst::TermDefn> {
    Some(nst::TermDefn {
...
mod tests {
  ...
  use test_utils::{
    ...
    nterm_match, nterm_prod, nterm_sum, nterm_var, nterm_with, quant, scheme, type_func, type_int,
    type_named, type_prod,
  };
  ...
  fn parse_resolve_module<'a, S: ToString>(
    ...
    let effects = nameres_module.effects(db).iter().map(|oe| {
      oe.map(|effect| {
        ...
        (
          test_utils::nst::Item::Effect(effect.data(db).clone().ref_alloc(&mut ref_alloc)),
          effect.locals(db),
        )
      })
    });
