  ...
  pub(crate) fn lower_term(&mut self, ast: &Ast<VarId>, term: Idx<Term<VarId>>) -> DelimReducIr {
    ...
    match ast.view(term) {
      ...
      Abstraction { arg, body } => {
        ...
        let mut vars = vec![ReducIrVar::new(
          ReducIrLocal {
            top_level: self.current,
      ...
      Operation(op) => {
        ...
        let kont_var =
          ReducIrVar::new(self.generate_local(), self.mk_fun_ty([op_ret], kont_ret_ty));
