...
mod tests {
  ...
  use base::{
    diagnostic::{error::PanoplyErrors, parser::ParseError}, file::{FileId, SourceFile, SourceFileSet}, ident::Ident, indexed::ReferenceAllocate, span::SpanOf
  };
  use bumpalo::Bump;
  ...
  fn parse_term<'a>(
    ...
    let mut parser = crate::parser2::Parser::new(tokens, eoi);
    let term = parser.term()?;
    let mut ref_alloc = CstRefAlloc::new(arena, &parser.alloc);
    Ok(term.ref_alloc(&mut ref_alloc))
  }

  fn parse_type<'a>(db: &'a dyn crate::Db, arena: &'a Bump, input: &str) -> &'a Type<'a, Ident> {
    let file_id = FileId::new(db.as_core_db(), PathBuf::from("test"));
    let (tokens, eoi) = lexer(db)
      .lex(&Locator::new(file_id, input), input)
      .expect("Lexing input failed");
    ...
    let mut parser = crate::parser2::Parser::new(tokens, eoi);
    let type_ = parser.type_().unwrap();
    let mut ref_alloc = CstRefAlloc::new(arena, &parser.alloc);
    type_.ref_alloc(&mut ref_alloc)
  }
  ...
  fn parse_scheme<'a>(
    ...
    let mut parser = crate::parser2::Parser::new(tokens, eoi);
    let scheme = parser.scheme().unwrap();
    let mut ref_alloc = CstRefAlloc::new(arena, &parser.alloc);
  ...
  fn test_product_types() {
    let db = TestDatabase::default();
    assert_matches!(parse_type(&db, &Bump::new(), "{}"), type_prod!());
    assert_matches!(
        parse_type(
            &db,
            &Bump::new(),
  ...
  fn test_unsign_int() {
    let db = TestDatabase::default();
    assert_matches!(
        parse_term(&db, &Bump::new(), "12354").unwrap(),
    ...
    assert_matches!(
        parse_term(&db, &Bump::new(), "00101").unwrap(),
        &Term::Int(SpanOf { value, .. }) => {
            assert_eq!(value, 101);
        }
    );
    ...
    assert_matches!(
        parse_term(&db, &Bump::new(), "0").unwrap(),
        &Term::Int(SpanOf { value, .. }) => {
            assert_eq!(value, 0);
        }
    );
  ...
  fn test_product_rows() {
    let db = TestDatabase::default();
    assert_matches!(parse_term(&db, &Bump::new(), "{}").unwrap(), term_prod!());
    assert_matches!(
        parse_term(
            &db,
            &Bump::new(),
  ...
  fn test_combined_postfixes() {
    let db = TestDatabase::default();
    assert_matches!(
        parse_term(&db, &Bump::new(), "a.x(b)").unwrap(),
  ...
  fn test_concat_variables() {
    let db = TestDatabase::default();
    assert_matches!(
        parse_term(&db, &Bump::new(), "x ,, y ,, z").unwrap(),
...
"#.to_string(),
  ...
  fn test_parse_wand() {
    ...
    let file = SourceFile::new(
      &db,
      FileId::new(&db, PathBuf::from("wand")),
      r#"defn f = |m||n| (m ,, n).x
...
"#.to_string(),
    ...
    let module = db.parse_module(file);
    dbg!(crate::parse_module::accumulated::<PanoplyErrors>(&db, file));
    let expect = expect![[r#"
        CstModule {
            indices: CstIndxAlloc {
                types: Arena {
