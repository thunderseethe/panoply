      ...
      .all(|pat| matches!(pat, nst::Pattern::Whole(_)))
    {
      ...
      let mut matches = constrs.into_iter().map(|(c, p)| match c {
        Constructor::ProductRow(ref lbls) => {
          ...
          let init =
            self.desugar_pattern_matrix(occs.as_mut_slice(), matrix.specialize(&c, self.arenas))?;
        ...
        Constructor::SumRow(label) => {
          ...
          occs.extend(occurences.iter_mut().skip(1).map(|var| *var));
          let func = self.desugar_pattern_matrix(&mut occs, matrix.specialize(&c, self.arenas))?;
...
impl Constructor {
  fn matches(&self, pat: &nst::Pattern, alloc: &NstIndxAlloc) -> Option<Vec<nst::Pattern>> {
    ...
    match (self, pat) {
      ...
      (Constructor::SumRow(lbl), nst::Pattern::SumRow(row)) if row.field.label.value.eq(lbl) => {
