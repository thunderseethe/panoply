...
"#
    ...
    fn lower_id() {
        ...
        let expect = expect!["(forall [(T1: Type) (T0: ScopedRow)] (fun [(V0: T0), (V1: T1)] V1))"];
    ...
    fn lower_product_literal() {
        ...
        let expect = expect![[r#"
            (forall [(T1: Type) (T0: ScopedRow)] (let
                ((V1: { T1 -> T1 -> {T1, T1}
                      , forall Type . (T2 -> T0) -> (T2 -> T0) -> <T2,T2> -> T0
                      , {{T1, T1} -> T1, T1 -> <T1,T1>}
                      , {{T1, T1} -> T1, T1 -> <T1,T1>}
                      }) ((_row_simple_x_y @ T1) @ T1))
                (fun [(V0: T0), (V2: T1)] (V1[0] V2 V2))))"#]];
        expect.assert_eq(&pretty_ir);
...
with {
    ...
    return = |x| |s| {state = s, value = x},
        ...
        let expect = expect![[r#"
            (forall [(T2: Type) (T1: ScopedRow) (T0: ScopedRow)] (fun
                [(V1: { {1} -> { Int
                               , { {} -> ({} -> {} -> {{}, {}}) -> {} -> {{}, {}}
                                 , {} -> ({} -> {} -> {{}, {}}) -> {} -> {{}, {}}
                                 }
                ...
                (let
                  ...
                  (fun [(V0: T1)]
                    (let
                      ...
                      (new_prompt [V6] (let ((V0: {0}) (V1[0] V0 {V6, (V4[2][0] V7)}))
                        (prompt V6 (V4[3][0]
                            V7
                            ((fun
                              [(V17: { Int
